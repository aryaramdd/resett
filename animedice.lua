...
-- SETTING FARMING: potion (boost) + trait reroll, TANPA dipakai bot
-- Potion = item kind Boost -> dimatikan lewat Boost.Enabled = false (jadi cuma dikumpulin)
-- Spin (Lucky/Jackpot) ikut mati karena pemakaian spin ada di modul Boost yang sama
-- Trait Reroll / Gems AMAN secara bawaan: core SENGAJA tidak menyambung remote
-- roll trait/grade (lihat komentar "CỐ Ý KHÔNG nối remote" di core), jadi tidak akan kepakai
-- Trait Reroll justru di-FARM lewat Tower drop + TicketShop (tukar tiket)
getgenv().AnimeDiceConfig = {
	Groups = { Farm = true, Economy = true, Content = true, Utility = true },
	UI = { Enabled = true, StartMinimized = false, ToggleKey = Enum.KeyCode.RightControl, LogLines = 60 },
	Roll = { Enabled = true, ServerAuto = false, ExtraDelay = 0.05, StopWhenFull = true },
	Collect = { Enabled = true, Interval = 1.0 },
	Plot = { Enabled = true, Interval = 4, CompareMode = "normalized", SwapMargin = 0.02, SwapPaybackSeconds = 600, ForceSwapRatio = 5, UseEquipBest = false },
	LevelUp = { Enabled = true, Interval = 2.5, MaxPaybackSeconds = 300, LockedPaybackSeconds = 60, MaxMoneyFraction = 0.35, MaxLevel = 0 },
	Sell = { Enabled = true, Interval = 2, ServerAutoSell = true, BatchSize = 60, UnlockWeakSecrets = true, UseIncomeGuard = true },
	Quest = { Enabled = true, Interval = 3, ForceTower = true, PushSell = true },
	Economy = { Enabled = true, Interval = 2.5, Priority = { "Dice", "RollSpeed", "Luck", "Damage", "Money", "Storage", "Health", "Sell", "Walkspeed" }, FallThroughFraction = 0.25 },
	Dice = { Enabled = true, MaxTier = "", AutoEquipBest = true, ReserveRatio = 0.6 },
	Upgrade = { Enabled = true, WalkspeedMaxTier = 1 },
	Rebirth = { Enabled = true, Mode = "smart", Margin = 1.0, Max = 0, VsDiceRatio = 1.0, UnlockFirst = true },
	Tower = {
		Enabled = true, Interval = 3.2, Mode = "auto", Fixed = "Dragon Tower", MinFloors = 5, AutoTeam = true,
		FarmBoosts = true, BoostStockTarget = 5,
		ResourceWeights = { Gems = 1, ["Trait Reroll"] = 1 },
		BoostWeight = 0.08, MaxSimFloors = 1500, LatencyMargin = 0.08, RefillEvery = 4, BoostBeforeRun = true,
	},
	Rewards = { Enabled = true, Interval = 30, Daily = true, Group = true, Offline = true, Quests = true, GroupMaxTries = 3 },
	Codes = { Enabled = true, Interval = 120, List = { "100KLIKES", "40KCCU", "30KCCU", "UPDATE4", "20KCCU", "10KCCU", "5KCCU", "1KCCU", "UPDATE3", "UPDATE2", "UPDATE1", "RELEASE" }, MaxTries = 3 },
	AntiAFK = { Enabled = true },
	Watchdog = { Enabled = true, Interval = 10, StuckSeconds = 300, Action = "rejoin", RequireBoth = true },
	LockedItems = {},
	Boost = {
		Enabled = false, Interval = 3,
		UseLuck = true, UseMoney = true, UseDamage = true, UseSpins = true,
		UseAll = true, MaxPerTick = 6, SpinOrder = "low",
	},
	Keep = { Interval = 0.5 },
	Webhook = { Enabled = false, URL = "", MinRarity = "Celestial", NotifyExisting = false, Interval = 2, MaxQueue = 100, MaxAttempts = 5 },
	TicketShop = { Enabled = true, Interval = 0.4, Reserve = 0, Items = { "Trait Reroll", "Gems" }, Mode = "rotate", AllowGamepass = false },
	AccountSwap = { Enabled = false, Interval = 5, Item = "Jackpot Spin", Amount = 1, FromFolder = "", ToFolder = "", Replace = false, ConfigId = nil, KeepItem = true },
	...
	WorkspaceCleanup = {
		...
		Aggressive = true, OtherCharacters = true,
		Limiteds = false, DebrisChildren = false, ClearTerrain = false, Zones = false,
		StripEffects = true, StripBatch = 400, StripOwnPlot = true, HideKeptParts = true,
		PlotPositions = true, Rigs = true, Lighting = true,
	},
}

-- ===== CORE MULAI DI BAWAH (hasil copy Anime Dice.txt) =====
...
local CFG = {
	...
	Codes = {
		...
		List = { "100KLIKES", "40KCCU", "30KCCU", "UPDATE4", "UPDATE5", "20KCCU", "10KCCU",
	...
	Keep = { Interval = 0.5 },
	Webhook = {
		Enabled = true, URL = "", -- chồng điền webhook Discord của mình ở config
