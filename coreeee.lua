...
local CFG = {
	...
	Tower = {
		...
		Mode      = "fixed",  -- "auto" = tự chọn tower lời nhất | "fixed"
		Fixed     = "Infinity Tower",
	...
	Keep = { Interval = 0.5 },
	Webhook = {
		Enabled = false, URL = "", -- webhook diisi di loader
...
function UI.build()
	...
	if CFG.UI.AIHub and CFG.UI.AIHub.Enabled ~= false then
		UI.hubToggleKey = CFG.UI.AIHub.ToggleKey or Enum.KeyCode.F1
		local hub = mk("Frame", {
			Name = "AIHubOverlay",
	...
	Util.track(Svc.UIS.InputBegan:Connect(function(input, gpe)
		...
		if UI.hub and UI.hubToggleKey and input.KeyCode == UI.hubToggleKey then
			local show = not UI.hub.Visible
			UI.hub.Visible = show
			main.Visible = not show and not CFG.UI.StartMinimized
			return
		end
		if input.KeyCode == CFG.UI.ToggleKey then
			if UI.hub and UI.hub.Visible then
				UI.hub.Visible = false
				main.Visible = not CFG.UI.StartMinimized
			else
...
function UI.refresh()
	...
	if not ((UI.main and UI.main.Visible) or (UI.hub and UI.hub.Visible)) then return end
	local r = UI.rows
	...
	local income = Game.plotIncome()
	set("income", Util.fmt(income) .. "/s")
	...
	local uptimeText = string.format("%02d:%02d:%02d", math.floor(up / 3600), math.floor(up % 3600 / 60), up % 60)
	set("uptime", uptimeText)
	if UI.hubNickname then UI.hubNickname.Text = LP.Name end
	if UI.hubUptime then UI.hubUptime.Text = "UPTIME | " .. uptimeText end
	if UI.hubStats then
		local statusNames = {
			["tắt"] = "OFF",
			["đang quay"] = "ROLLING",
			...
			["chờ ngưỡng bán an toàn"] = "WAITING SELL THRESHOLD",
			["đã tối ưu"] = "OPTIMIZED",
			["chưa đủ sức"] = "NOT ENOUGH POWER",
			["chờ thao tác pet"] = "WAITING PET ACTION",
			["chưa có ngưỡng plot"] = "NO PLOT LIMIT",
			["chờ đồng bộ pet plot"] = "WAITING PLOT SYNC",
			["chưa có pet plot với chance hợp lệ"] = "NO VALID PLOT PET",
			["không có pet dư dưới ngưỡng"] = "NO EXTRA PETS",
			["thiếu remote SellInventory"] = "MISSING SELL REMOTE",
			["bán lỗi/chưa xác nhận số lượng"] = "SELL ERROR",
		}
		local function statusText(value)
			local text = tostring(value or "-")
			if statusNames[text] then return statusNames[text] end
			if text:find("đã bán", 1, true) == 1 then return "SOLD" end
			if text:find("chờ gỡ khóa", 1, true) == 1 then return "UNLOCKING LOCKS" end
			if text:find("bán", 1, true) == 1 then return "SELLING" end
			return text
		end
		local towerText = tostring(RT.towerName or "-")
		if RT.towerFloor and RT.towerFloor > 0 then towerText = towerText .. " | Floor " .. tostring(RT.towerFloor) end
		UI.hubStats.Text = table.concat({
			"Roll : " .. statusText(RT.status.Roll) .. "  |  Rolls : " .. tostring(Util.data("Rolls", 0)) .. " ( OK " .. tostring(RT.rollOk or 0) .. " )",
			"MONEY : " .. Util.fmt(money) .. "  |  INCOME/S : " .. Util.fmt(income) .. "/S",
			"UNITS : " .. tostring(Game.unitCount()) .. " / " .. tostring(math.floor(Game.storageCap())) .. "  |  PLOTS : " .. tostring(#Game.unlockedSlots()) .. "  |  LUCK : x" .. Util.fmt(Util.buff("Luck", 1)),
			"DICE : " .. tostring(dname) .. "  |  TOWER : " .. towerText .. "  |  SELL : " .. statusText(RT.status.Sell),
			"BOOST : " .. tostring(RT.boostText or "-") .. "  |  PETS SOLD : " .. tostring(RT.sold or 0) .. " (" .. Util.fmt(RT.soldCash or 0) .. ")",
		}, "\n")
	end
...
local function boostTick()
	...
	RT.boostText = string.format("Luck %d layers | Money %d | Damage %d", nl, ni, nd)
