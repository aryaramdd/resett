-- =====================================================================
-- Peak ETFB Core FastCombine (No UI)
-- Sama persis dengan Peak_ETFB_Core.lua, TAPI logic combine mesin diubah:
--   1. Setelah deposit LANGSUNG combine (ga balik ke base dulu),
--      biar brainrot ga kelewat/tidak ke-combine gara2 balik ke base.
--   2. Cooldown cuma jalan kalau combine BENAR2 berhasil. Kalau brainrot
--      ga ke-combine (isi mesin masih ada), langsung coba lagi tanpa cooldown.
-- Semua fitur dikendalikan lewat getgenv().CONFIG (diset di loader).
-- Upload file ini ke GitHub, lalu panggil lewat loader:
--   loadstring(game:HttpGet("https://raw.githubusercontent.com/.../Peak_ETFB_Core_FastCombine.lua"))()
-- =====================================================================
...
-- ====== SOLO SERVER ANTI-LOOP ======
if cfg("Base", "SoloServer") == true then
    getgenv().HasTeleportedToSolo = getgenv().HasTeleportedToSolo or false
    if not getgenv().HasTeleportedToSolo then
        local player = game:GetService("Players").LocalPlayer
        local autoRejoinWarning = player.PlayerGui:WaitForChild("AutoRejoinWarning", 10)
        
        if autoRejoinWarning and autoRejoinWarning.Enabled == true then
            getgenv().HasTeleportedToSolo = true
            print("Sudah di Solo Server.")
        else
            getgenv().HasTeleportedToSolo = true
            print("Pindah ke Solo Server...")
            local RS = game:GetService("ReplicatedStorage")
            pcall(function()
                RS.Shared.Remotes.Networking["RE/Misc/RejoinServer"]:FireServer()
            end)
            return -- Hentikan eksekusi Core karena akan pindah server
        end
    end
end
...
task.spawn(function()
	while task.wait() do
		...
		pcall(function()
			for key3, value3 in pairs(brainrots:GetDescendants()) do
...
task.spawn(function()
	while task.wait() do
		local luckyBlocks = workspace:FindFirstChild("ActiveLuckyBlocks")
		if not luckyBlocks then continue end
		pcall(function()
			for key4, value4 in pairs(luckyBlocks:GetChildren()) do
...
-- ====== MAP KILLER (pengganti FPS boost lama) ======
-- Hapus terus map/visual yang muncul di Workspace (tiap event ganti map),
-- BasePlate, ActiveBrainrots/LuckyBlocks/Tsunamis, brainrot di base sendiri,
-- dan efek Lighting. Dikontrol: CONFIG.FPSBoost.Enabled & DeleteBaseBrainrot.
if FPSBoostEnabled then
	local PROTECTED = {
		...
		BlackBox = true,
		NPCDebug = true,
		SummonPlatformBlocks = true,
		PaintWorldBGFX = true,
	}
	local KILL_EXACT = {
		BasePlate1 = true,
		BasePlate2 = true,
		ActiveBrainrots = true,
		ActiveLuckyBlocks = true,
		ActiveTsunamis = true,
	}

	local function isMapLikeName(name)
		if type(name) ~= "string" then return false end
		if name:find("_Visual$") then return true end
		if name:find("Wave%d") or name:find("LowWave") or name:find("WackyWave")
			or name:find("SnakeWave") then return true end
		if name:find("Map") and not name:find("MapVariant") then return true end
		if name:find("SummerMap") or name:find("_SharedInstances$") then return true end
		return false
	end

	local function shouldKill(model)
		if not model:IsA("Model") then return false end
		if model.Parent == workspace and KILL_EXACT[model.Name] then return true end
		if PROTECTED[model.Name] then return false end
		local p = model.Parent
		while p and p ~= workspace do
			if PROTECTED[p.Name] then return false end
			p = p.Parent
		end
		if model:FindFirstChild("Humanoid") then return false end
		if isMapLikeName(model.Name) then return true end
		return false
	end

	local function killMap(model)
		pcall(function()
	...
	local function canKill(inst)
		if inst:IsA("Model") then
			return shouldKill(inst)
		elseif inst:IsA("Folder") then
			if inst.Parent == workspace and KILL_EXACT[inst.Name] then return true end
			if inst.Parent == workspace and (inst.Name:find("_Visual") or inst.Name:find("Map")) then
				return true
			end
		end
		return false
	...
	local function killLightingFx()
		game.Lighting.GlobalShadows = false
		game.Lighting.FogEnd = 100000
		game.Lighting.FogStart = 0
		pcall(function() game.Lighting.FogColor = Color3.fromRGB(150, 150, 150) end)
		...
		for _, child in pairs(game.Lighting:GetChildren()) do
			pcall(function()
				if child:IsA("Atmosphere") or child:IsA("BloomEffect") or child:IsA("BlurEffect")
					...
					or child:IsA("DepthOfFieldEffect") or child:IsA("Sky") or child:IsA("Clouds")
					or child:IsA("GodRays") then
					child:Destroy()
				end
			end)
		end
	...
	local function killOtherBasesAndOwnBrainrot()
		local myUserId = players.LocalPlayer.UserId
		if not workspace:FindFirstChild("Bases") then return end
		for _, b in pairs(workspace.Bases:GetChildren()) do
			local holder = b:GetAttribute("Holder")
			if holder then
				if holder == myUserId then
					-- Base sendiri: diselamatkan, tapi bersihkan brainrot untuk FPS boost
					if DeleteBaseBrainrot then
						for _, c in pairs(b:GetChildren()) do
							if string.find(c.Name, "brainrot") then
								pcall(function() c:Destroy() end)
							end
						end
					end
				else
					-- Base orang lain: hapus total!
	...
	local function killMaster()
		killLightingFx()
		killOtherBasesAndOwnBrainrot()
		...
		for _, m in pairs(workspace:GetDescendants()) do
			if m:IsA("Model") and m.Parent ~= workspace and shouldKill(m) then
				killMap(m)
			end
		end
	end

	task.spawn(killMaster)

	workspace.DescendantAdded:Connect(function(inst)
		task.spawn(function()
			task.wait(0.05)
			if canKill(inst) then
				killMap(inst)
			end
		end)
	end)
	...
	task.spawn(function()
		...
		local function step()
			...
			if STATE == "DEPOSITED" then
				-- LANGSUNG combine: ga balik ke base dulu, biar brainrot-nya kepasti ke-combine.
				-- (kadang kalau balik ke base dulu, brainrot malah ga ke-combine)
				...
				-- ada isi: LANGSUNG COMBINE dari posisi mesin (ga balik base)
				...
				task.wait(0.3)
				local ok = pcall(function()
					return networkRemote:InvokeServer("Combine", currentMachine)
				end)

				-- Verifikasi hasil: kasih waktu server proses, lalu cek apakah
				-- isi mesin beneran habis (brainrot ke-combine). Kalau masih ada,
				-- berarti combine gagal -> coba lagi tanpa cooldown.
				task.wait(0.8)
				local machineStill = currentMachine
				if not machineStill or not machineStill.Parent then
					machineStill = spawnMachinesFolder:FindFirstChild(currentMachineName or "")
				end
				local stillFilled = machineStill and hasContent(machineStill) == true

				if ok and not stillFilled then
					-- combine BERHASIL: isi mesin habis -> baru cooldown
					STATE = "COOLDOWN"
					...
					currentMachineName = nil
					tweenTo(getBasePos())
					log("Combine SUKSES, cooldown " .. COOLDOWN_TIME .. "s")
				else
					-- combine belum berhasil (isi masih ada / remote gagal)
					-- tetep di DEPOSITED biar coba lagi, GA usah cooldown
					log("Combine belum sukses, coba lagi (ga cooldown)")
					emptyChecks = 0
					missingChecks = 0
					task.wait(0.5)
					return
...
task.spawn(function()
	...
	remote.OnClientEvent:Connect(function(popupType, ...)
		-- HANYA kirim popup Info (hasil combine machine) DAN mengandung kata DUPED.
		-- GlobalMessage / BrainrotSpawn / dll DILEWAT (biar ga spam Discord).
		if tostring(popupType) ~= "Info" then
			return
		end
		local args = { ... }
		local msg = args[1]
		...
		if txt ~= "" and string.find(string.upper(txt), "DUPED") then
